<template>
    ...
    <Head title="Personalizar Informe" />
    <AuthenticatedLayout>
        <ContentBox title="Personalizar Informe"
            description="Completa el formulario con los datos que quieras ver reflejados en el informe"
                ...
                <form @submit.prevent="submit"
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 p-2">
                            <div class="flex-1">
                                ...
                                <Datepicker id="start"
                                    ...
                                    v-model="form.dateStart" inputFormat="dd-MM-yyyy" :locale="localLanguage"
                                    :upperLimit="today" :lowerLimit="lastDay"
                            ...
                            <div class="flex-1">
                                ...
                                <Datepicker id="end"
                                    class="w-full border border-gray-300 bg-blue-100 text-lavender-dark focus:border-lavender-vlight focus:ring-lavender-vlight rounded-md shadow-sm transition duration-200"
                                    v-model="form.dateEnd" inputFormat="dd-MM-yyyy" :locale="localLanguage"
                                    :upperLimit="today" :lowerLimit="form.dateStart === null
                                            ? lastDay
                                            : form.dateStart
                    ...
                    <div class="mt-3">
                        <p class="text-white flex justify-between">
                            ...
                            <button @click="toggleAllZones" type="button"
                        ...
                        <div class="flex flex-wrap justify-center gap-3 p-4">
                            <label v-for="(zone, index) in props.zonas" :key="index"
                                class="flex text-center items-center justify-center cursor-pointer group transition-all duration-200">
                                <input :id="index" type="checkbox" :value="zone.id" v-model="form.zones"
                                    class="hidden peer" />
                                <span
                        ...
                        <div class="flex justify-center items-center">
                            ...
                            <InputError v-for="(messages, field) in form.errors" :key="field" :message="field.startsWith('zones.') ? messages : null
                                " />
                        </div>
                    </div>
                    ...
                    <div class="mt-3">
                        <p class="text-white flex justify-between">
                            ...
                            <button @click="toggleAllCenters" type="button"
                                class="cursor-pointer text-xs text-lavender-vlight underline hover:text-skyblue-vlight transition">
                                {{
                        ...
                        <div class="w-full h-1 rounded-xl bg-skyblue-logo" />
                        <div class="flex flex-wrap justify-center gap-3 p-4">
                            <label v-for="(center, index) in props.centros" :key="index"
                                class="flex text-center items-center justify-center cursor-pointer group transition-all duration-200">
                                <input :id="'c' + index" type="checkbox" :value="center.id" v-model="form.centers"
                                    class="hidden peer" />
                                <span
                                    ...
                                    <span class="font-bold">{{
                                        center.nombre
                                        }}</span>&nbsp;<span>({{ center.localidad }})</span>
                                </span>
                            </label>
                        </div>
                        <div class="flex justify-center items-center">
                            ...
                            <!--Este InputError muestra los posibles errores en la validación del back-->
                            <InputError v-for="(messages, field) in form.errors" :key="field" :message="field.startsWith('centers.')
                                    ? messages
                                    : null
                                " />
                        </div>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="flex flex-wrap justify-center gap-3 p-4">
                            <label key="meses"
                                class="flex text-center items-center justify-center cursor-pointer group transition-all duration-200">
                                <input id="rMes" type="radio" :value="true" v-model="form.period" class="hidden peer" />
                                <span
                                    class="w-full h-full px-2 py-1 text-sm font-bold border-2 border-lavender-dark rounded-md text-lavender-dark bg-blue-100 peer-checked:bg-lavender-light peer-checked:border-lavender-light shadow-sm transition-all duration-300 hover:bg-skyblue-light hover:border-lavender-light">
                                    Por Meses
                                </span>
                            ...
                            <label key="anios"
                                class="flex text-center items-center justify-center cursor-pointer group transition-all duration-200">
                                <input id="rAnio" type="radio" :value="false" v-model="form.period"
                                    class="hidden peer" />
                                <span
                                    class="w-full h-full px-2 py-1 text-sm font-bold border-2 border-lavender-dark rounded-md text-lavender-dark bg-blue-100 peer-checked:bg-lavender-light peer-checked:border-lavender-light shadow-sm transition-all duration-300 hover:bg-skyblue-light hover:border-lavender-light">
                                    Por Años
...
const submit = () => {
    if (validateForm()) {
        sendForm(() => {
            form.post(route("client.mostrarInforme"));
